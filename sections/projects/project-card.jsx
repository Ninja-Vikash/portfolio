...
function Description({ title, desc }) {
  return (
    <div className="project-desc">
      <h2 className="text-xl text-zinc-100 tracking-wide border-b pb-2" style={{fontFamily: "var(--font-300)"}}>{title}</h2>
...
function Icon({ icon , w, alt, padding, scale }) {
  ...
  return (
    <Image src={`/Icons/${icon}.svg`} width={w} height={20} alt={alt || "image"} className={`${style} ${hoverEffect} ease-in transition-all`}/>
  );
}
...
export function ProjectCard({image, title, desc, techStack, netlifyLink, githubLink}) {
  return (
    <>
      <div
        className="bg-gradient-to-br from-zinc-950/100 via-zinc-950/50 to-zinc-950/70 rounded-lg p-4 relative">
        ...
        <div className="icons flex items-center gap-7">
          {netlifyLink && <a target='_blank' href={netlifyLink}>
